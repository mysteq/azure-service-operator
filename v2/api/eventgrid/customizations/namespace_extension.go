...
func (ext *NamespaceExtension) ExportKubernetesSecrets(
	...
	if !ok {
		return nil, eris.Errorf("cannot run on unknown resource type %T, expected *eventgrid.Namespace", obj)
	}
	...
	primarySecrets := secretsSpecifiedForNamespace(typedObj)
	...
	var namespacesClient *armeventgrid.NamespacesClient
	namespacesClient, err = armeventgrid.NewNamespacesClient(subscription, armClient.Creds(), armClient.ClientOptions())
	if err != nil {
		return nil, eris.Wrapf(err, "failed to create new NamespacesClient")
	}

	var resp armeventgrid.NamespacesClientListSharedAccessKeysResponse
	resp, err = namespacesClient.ListSharedAccessKeys(ctx, id.ResourceGroupName, typedObj.AzureName(), nil)
	if err != nil {
		return nil, eris.Wrapf(err, "failed listing keys")
	}

	secretSlice, err := secretsToWriteForNamespace(typedObj, resp)
	if err != nil {
		return nil, err
	}
...
func secretsSpecifiedForNamespace(obj *storage.Namespace) set.Set[string] {
...
func secretsToWriteForNamespace(obj *storage.Namespace, keys armeventgrid.NamespacesClientListSharedAccessKeysResponse) ([]*v1.Secret, error) {
